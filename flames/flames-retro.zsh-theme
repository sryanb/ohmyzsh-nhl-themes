...
# Colors (using retro Calgary Flames palette)
PRIMARY='0'        # #000000 (Black)
SECONDARY='124'    # #E03A3E (Retro Red)
TERTIARY='220'     # #FFC52F (Retro Yellow/Gold)
ACCENT='15'        # #FFFFFF (White)
RESET='%f'

# Nerd Font Symbols
TEAM_ICON="\uf06d"         # Fire icon from Nerd Fonts
...
prompt_git() {
  ...
  if [[ "$(command git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    prompt_segment $TERTIARY $PRIMARY ''
    ...
    echo -n "%{%F{$PRIMARY}%}${${ref:gs/%/%%}/refs\/heads\//$PL_BRANCH_CHAR }%{%f%}${vcs_info_msg_0_%% }${mode}"
  fi
}
...
prompt_status() {
  ...
  [[ -n "$symbols" ]] && prompt_segment $PRIMARY $ACCENT "$symbols"
}
...
prompt_aws() {
  ...
  case "$AWS_PROFILE" in
    *-prod|*production*) prompt_segment $SECONDARY $ACCENT "AWS: ${AWS_PROFILE}" ;;
    *) prompt_segment $TERTIARY $PRIMARY "AWS: ${AWS_PROFILE}" ;;
  esac
}
